...
export function PrintLabelModal({ isOpen, onClose, record }: PrintLabelModalProps) {
  ...
  const getBatchLabels = (count: number): string[] => {
    const labels: string[] = [];
    for (let i = 0; i < Math.min(count, record.plantingCount); i++) {
      labels.push(generateLabelNumber(record.plantCode, i + 1));
    }
    return labels;
  };
  ...
  const handleExportExcel = () => {
    ...
    // 生成Excel内容（HTML格式）
    const headers = ['标签编号', '种植批号', '作物名称', '作物品种', '种植区域', '种植数量', '种植日期', '溯源码'];
    ...
    a.href = url;
    a.download = `种植标签_${record.plantCode}_${new Date().toISOString().slice(0, 10)}.xls`;
    a.click();
  ...
  const getQrCodeValue = (label: string) => JSON.stringify({
    ...
    quantity: record.plantingCount,
    site: record.areaName,
            ...
            <div className="flex items-center gap-4">
              <div>
                ...
                <input
                  type="number"
                  min="1"
                  max={record.plantingCount}
                  value={printCount}
                  onChange={(e) => setPrintCount(Math.max(1, Math.min(record.plantingCount, Number(e.target.value))))}
                  className="w-24 px-3 py-1 border border-gray-300 rounded text-sm"
                />
              </div>
              <div className="text-xs text-gray-500">
                将生成 {printCount} 个标签编号
              </div>
            </div>
          )}
        </div>
