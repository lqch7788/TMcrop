...
export default function IndicatorBudgetApproval() {
  ...
  const tabs = [
    ...
    { key: 'indicator_adjust', label: '指标调整', icon: BarChart3, types: [ApprovalType.INDICATOR_ADJUST] },
              ...
              <TableRow key={approval.id} className={selectedIds.has(approval.id) ? 'bg-emerald-50' : ''}>
                <TableCell>
                  {approval.status === ApprovalStatus.PENDING ? (
                    <Button variant="ghost" size="icon" onClick={() => handleToggleSelect(approval.id)}>
