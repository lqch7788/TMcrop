...
/** 稳定的空数组引用，避免 selector 返回新引用导致无限循环 */
const EMPTY_OCCUPATIONS: ScheduleOccupation[] = [];
...
export function useDispatchOccupations(date: string) {
  // ★ 修复 getSnapshot 无限循环：selector 返回稳定引用（不可用 ?? [] 每次新建）
  const occupationMap = useScheduleStore((s) => s.occupations);
  ...
  const occupations = useMemo(
    () => occupationMap[date] ?? EMPTY_OCCUPATIONS,
    [occupationMap, date]
  );
