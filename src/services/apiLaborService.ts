...
// enhancedApiClient 内置三级降级（API → IndexedDB → localStorage）
// 无需额外 localStorage 配置
...
export async function getWorkers(filters?: EmployeeFilter): Promise<Worker[]> {
  ...
  const data = await enhancedApiClient.get<Worker[]>('/labor/workers', params);
  return data || [];
}
...
export async function getWorkerStats(): Promise<{
  ...
  return enhancedApiClient.get('/labor/workers/stats');
}
...
export async function getWorkerSkillTags(): Promise<string[]> {
  return enhancedApiClient.get<string[]>('/labor/workers/skill-tags');
}
...
export async function getWorkerTrainingRecords(workerId: string): Promise<any[]> {
  return enhancedApiClient.get<any[]>(`/labor/workers/${workerId}/training-records`);
}
...
export async function getWorkerAssessmentRecords(workerId: string): Promise<AssessmentRecord[]> {
  return enhancedApiClient.get<AssessmentRecord[]>(`/labor/workers/${workerId}/assessment-records`);
}
...
export async function getWorkerWorkExperiences(workerId: string): Promise<WorkExperience[]> {
  return enhancedApiClient.get<WorkExperience[]>(`/labor/workers/${workerId}/work-experiences`);
}
...
export async function generateWorkerId(): Promise<string> {
  return enhancedApiClient.get<string>('/labor/workers/generate-id');
}
...
export async function importWorkers(workers: CreateEmployeeParams[]): Promise<{ success: number; failed: number }> {
  return enhancedApiClient.post('/labor/workers/import', { workers });
}
...
export async function exportWorkers(filters?: EmployeeFilter): Promise<Blob> {
  return enhancedApiClient.get('/labor/workers/export');
}
