...
## enhancedApiClient (缓存层)

位于 `src/lib/apiClient.ts`。封装了 IndexedDB 缓存 + HTTP 请求:

- **cache-first**: 优先返回缓存，后台刷新
- **自动缓存**: GET 请求结果自动写入 IndexedDB
- **token 注入**: 自动从 localStorage 读取 token 附加 Authorization header

多数基础数据 Store 通过 `apiBasicDataService.ts` 间接使用 enhancedApiClient。
