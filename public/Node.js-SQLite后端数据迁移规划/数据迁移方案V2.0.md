...
| 问题 | 现状 | 理想状态 |
|------|------|---------|
| 种源→育苗 | `sourceId` 硬编码引用，但育苗数据里的 `sourceId` 如果改了，种源表不会知道 | 外键关联，级联更新 |
...
class MigrationSafetyManager {
  ...
  private shouldRollback(): boolean {
    return (
      this.errorCount >= 3 ||
      (Date.now() - this.lastErrorTime) < 60000
    );
  }
...
function backupLocalStorage(): BackupResult {
  ...
  return { success: true, keyCount: Object.keys(backup).length };
}
...
async function verifyDataConsistency(): Promise<ConsistencyReport> {
  const report: ConsistencyReport = {
    totalRecords: 0, matchedRecords: 0,
    mismatchedRecords: [], missingInSQLite: [], missingInLocalStorage: [],
  };
  ...
  for (const key of ALL_STORAGE_KEYS) {
    ...
    if (!localData && sqliteData) {
      ...
      if (JSON.stringify(local) === JSON.stringify(sqliteData)) {
        ...
        report.mismatchedRecords.push({ key, localValue: local, sqliteValue: sqliteData });
      }
    }
  }
