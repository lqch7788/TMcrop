...
| 作物管理 | 采收入库 | HarvestPage.tsx | ⚠️ 有审批但联动不完整 | 🟡 中风险：审批类型存在但业务联动不完整 |
...
| 文件路径 | 修改内容 |
|----------|----------|
| `src/types/approval.ts` | 扩展ApprovalType枚举至32种+扩展BusinessLink |
| `src/pages/Approvals.tsx` | 扩展支持全部32种审批类型Tab+批量审批UI |
| `src/App.tsx` | 删除冗余路由，新增/my-applications |
| `src/components/layout/Sidebar.tsx` | 精简菜单 |
| `src/types/approvalIntegration.ts` | 实现全部32种联动处理器 |
...
| 1.2 | 扩展 Approvals.tsx 支持全部32种审批类型Tab | 高 |
...
interface XxxStore {
  items: Xxx[];
  updateStatus: (id: string, status: XxxStatus) => Promise<void>;
  // 其他业务方法...
}
```
...
| V4.0 | 2026-05-04 | 融合`审批流程优化-V1.0.md`，新增批量审批、分级机制、通知、超时委托等高级功能，审批类型扩展至37种 |
| V4.1 | 2026-05-04 | 补充完整内容：系统深入分析、权限矩阵设计、通知机制、超时委托、UI设计、数据迁移方案、优化建议等，章节扩展至20章 |
