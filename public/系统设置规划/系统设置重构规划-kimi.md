# TMcrop 种植管理系统 — 系统设置模块全面诊断与重构方案

> **诊断版本**：V3.0 深度联动版  
> **诊断日期**：2026-05-02  
> **诊断范围**：系统设置全部入口页面（20个）、子页面（50+个）、数据流、与业务模块的关联引用  
> **核心目标**：让设置模块成为"数据中央厨房"——后台一改，全局联动  
...
2. [现状全景扫描](#二现状全景扫描)
3. [问题分级诊断](#三问题分级诊断)
4. [模块合并与拆分方案](#四模块合并与拆分方案)
5. [模块升级详细设计](#五模块升级详细设计)
6. [数据联动架构设计](#六数据联动架构设计)
7. [SQLite Schema 正规化改造](#七sqlite-schema-正规化改造)
8. [数据迁移与清洗方案](#八数据迁移与清洗方案)
9. [实施路线图](#九实施路线图)
10. [预期效果对比](#十预期效果对比)
...
- "作物管理"在入口B中，但路由是 /settings/crops → 实际指向 CropManagement（业务页面，非设置）
```

### 2.2 已确认存在的设置页面（20个路由全部有文件）

| # | 路由 | 实际文件 | 持久化方式 | 能否CRUD | 状态 |
|---|------|---------|-----------|---------|------|
| 1 | /settings/system-config | SystemConfig.tsx | LocalStorage | ✅ 能增删改 | 🟡 可用但本地存储 |
| 2 | /settings/dictionary | DictionaryManagement.tsx | 硬编码+LocalStorage | ✅ 能增删改 | 🟡 字典值写死在代码 |
| 3 | /settings/user-permission | UserPermission.tsx | 硬编码 | ❌ 不能改 | 🔴 纯展示 |
| 4 | /settings/approval-workflow | ApprovalWorkflowConfig.tsx | LocalStorage | ✅ 能增删改 | 🟡 角色字符串无法解析 |
| 5 | /settings/notification | NotificationSettings.tsx | LocalStorage | ✅ 能增删改 | 🟡 收件人无法指定具体人 |
| 6 | /settings/device | DeviceManagement.tsx | LocalStorage | ✅ 能增删改 | 🟡 可用但本地存储 |
| 7 | /settings/warehouse | WarehouseManagement.tsx | LocalStorage | ✅ 能增删改 | 🟡 可用但本地存储 |
| 8 | /settings/team | TeamManagement.tsx | 未知 | 未读取 | ⚠️ 待确认 |
| 9 | /settings/branch | BranchManagement.tsx | 未知 | 未读取 | ⚠️ 待确认 |
| 10 | /settings/block | BlockManagement.tsx | 未知 | 未读取 | ⚠️ 待确认 |
| 11 | /settings/bases | BaseSettings.tsx | 硬编码 | ❌ 不能持久化 | 🔴 页面有但数据写死 |
| 12 | /settings/regions | PlantAreaManagement.tsx | 未知 | 未读取 | ⚠️ 待确认 |
| 13 | /settings/modes | PlantingModeManagement.tsx | 未知 | 未读取 | ⚠️ 待确认 |
| 14 | /settings/crop-variety | CropVarietyManagement.tsx | 未知 | 未读取 | ⚠️ 待确认 |
| 15 | /settings/materials | MaterialManagement.tsx | 未知 | 未读取 | ⚠️ 待确认 |
| 16 | /settings/processes | ProcessManagement.tsx | 未知 | 未读取 | ⚠️ 待确认 |
| 17 | /settings/personnel | PersonnelManagement.tsx | 硬编码 | ❌ 不能持久化 | 🔴 纯展示+跳转 |
| 18 | /settings/departments | DepartmentSettings.tsx | 硬编码 | ❌ 不能持久化 | 🔴 数据写死 |
| 19 | /settings/cost-accounting | CostAccounting.tsx | 未知 | 未读取 | ⚠️ 待确认 |
| 20 | /settings/audit-log | AuditLog.tsx | 未知 | 未读取 | ⚠️ 待确认 |

**重要发现**：所有20个路由对应的页面文件 **全部存在**！之前诊断误判为"有路由无页面"，实际是每个页面都有文件，但部分页面功能不完整（纯展示/硬编码）。
...
│ 导出方式     │ export default      │ export default      │
└──────────────┴─────────────────────┴─────────────────────┘

结论：两个文件由不同开发者在不同时期创建，内容几乎完全相同，
      但文件A在components目录下，文件B在pages目录下。
      系统实际使用的是文件B（路由直接指向pages目录）。
```
...
│ 数据状态     │ useState            │ useState            │
└──────────────┴─────────────────────┴─────────────────────┘

结论：文件A和B内容几乎完全相同，文件A有新增/编辑弹窗的完整实现，
      文件B的弹窗实现也很完整。两者都使用硬编码数据。
```
...
│    维度      │      文件A          │      文件B          │
├──────────────┼─────────────────────┼─────────────────────┤
│ 数据定义     │ const staffData     │ const staffData     │
│             │ (完全相同的6人)      │ (完全相同的6人)      │
│ 页面标题     │ "人员管理"          │ "人员管理"          │
│ 搜索功能     │ 姓名/工号          │ 姓名/工号          │
│ 部门筛选     │ 下拉选择            │ 下拉选择            │
...
│ 表格列       │ 9列                │ 9列                │
│ 分页逻辑     │ 相同               │ 相同               │
│ 路由入口     │ /settings/personnel │ 独立路由            │
└──────────────┴─────────────────────┴─────────────────────┘

文件C内容：
export { PersonnelPage, default } from '../components/personnel';
...
位置5: src/components/labor/attendance/WorkerAttendancePage.tsx
  ...
  ↑ 又不一样了：没有设备部，多了"全部"选项

位置6: src/components/personnel/personnelPage.tsx
  筛选下拉：['全部','生产部','技术部','后勤部']
  ↑ 又不一样了：没有管理层/财务部/质检部/仓储部/设备部
...
**问题严重性**：同一个"生产部"在不同页面可能有不同的名字或不同的部门集合。如果用户想新增一个"质检部"，需要改 **5个文件**；如果用户想修改"技术部"为"技术工程部"，需要改 **7个位置**，而且容易遗漏。
...
位置3: src/components/labor/tempTask/TempTaskFormModal.tsx
  const greenhouses = [...]  ← 独立的温室数组

位置4: src/components/labor/tasks/BatchEditModal.tsx
  const greenhouses = [...]  ← 独立的温室数组

位置5: src/components/production/modals/BatchEditModal.tsx
  const greenhouses = [...]  ← 独立的温室数组

位置6: src/components/farm/harvest/HarvestPage.tsx
  const greenhouses = [...]  ← 独立的温室数组

位置7: src/components/farm/hub/InspectionTab.tsx
  const greenhouses = [...]  ← 独立的温室数组
```
...
-- ... 类似的反模式表还有：
-- bases, greenhouses, plant_areas, warehouses, materials, 
-- system_configs, dictionaries, planting_modes, blocks, indicators,
-- farm_activities, produce_inventories, attendance_records, etc.
```

**反模式后果**：
1. 无法SQL查询：`SELECT * FROM departments WHERE name = '生产部'` → 失败，因为name在JSON里
2. 无法建索引：无法在JSON字段上建索引
3. 无法外键关联：`FOREIGN KEY (department_id) REFERENCES departments(id)` → 可以，但无法JOIN查询属性
4. 数据校验困难：无法利用数据库的NOT NULL/UNIQUE约束
5. 前端需要手动解析JSON，增加复杂度
...
#### 新增1：组织架构（合并公司+基地+温室+区域+仓库+部门）

**为什么合并**：
- 这6个模块都是"空间/组织"维度的基础数据
- 它们在业务中经常被同时引用（种植要选择基地→温室→区域）
- 合并后可以在一个页面维护所有空间维度数据，减少跳转
...
│  │  搜索 [________]                    [新增公司] [导入]      │  │
│  ├──────────────────────────────────────────────────────────┤  │
│  │  公司编码 │ 公司名称    │ 负责人  │ 基地数 │ 人员数 │ 状态  │  │
│  │  ───────────────────────────────────────────────────────  │  │
│  │  CG001   │ 宁波帮帮忙  │ 张三   │   4    │  32   │ 正常  │  │
│  │  CG002   │ 成都帮帮您  │ 李四   │   7    │  28   │ 正常  │  │
│  └──────────────────────────────────────────────────────────┘  │
│                                                                 │
│  点击公司行 → 展开详情：                                         │
│  ┌──────────────────────────────────────────────────────────┐  │
...
export function StaffSelect({ value, onChange, mode = 'single', departmentId }: StaffSelectProps) {
  // 从全局状态获取人员列表（自动过滤在职）
  ...
  return (
    <Select value={value} onChange={onChange} mode={mode}>
      {filteredStaff.map(staff => (
        <Option key={staff.id} value={staff.id}>
          <Avatar src={staff.avatar} size="small" />
          <span>{staff.name}</span>
          <Tag>{staff.departmentName}</Tag>
        </Option>
      ))}
    </Select>
  );
...
// 使用示例：
// <StaffSelect 
//   value={formData.createById} 
//   onChange={id => setFormData({...formData, createById: id})}
// />
// → 人员库新增员工后，此下拉自动出现新员工
```
...
export function DictSelect({ categoryCode, value, onChange }: DictSelectProps) {
  const [options, setOptions] = useState<DictItem[]>([]);
  
  useEffect(() => {
    // 从全局字典缓存获取
    const items = useSettingsStore.getState().getDictItems(categoryCode);
    setOptions(items);
  }, [categoryCode]);
...
export const useSettingsStore = create<SettingsState>((set, get) => ({
  ...
  loadAll: async () => {
    // 并行加载所有设置数据
    const [companies, bases, greenhouses, plantAreas, warehouses, departments, staffList] = 
      await Promise.all([
    ...
    set({ companies, bases, greenhouses, plantAreas, warehouses, departments, staffList });
  },
...
('SC013', 'task_execution_critical_hours', '任务执行危急时间', '48', 'number', 'business', '任务执行超时危急（小时）'),
...
// 8.2.2 清洗采收表的 harvesterNames（字符串数组→ID数组）
...
// 8.2.3 清洗采收表的 warehouseName（字符串→ID）
...
├── src/pages/PositionManagement.tsx                → 合并到PersonnelManagement（子Tab）
...
├── src/pages/PlantAreaManagement.tsx               → 合并到BaseSettings（子Tab）
├── src/pages/BranchManagement.tsx                  → 合并到BaseSettings（公司管理）
├── src/pages/BlockManagement.tsx                   → 合并到BaseSettings（区块→种植区域）
├── src/pages/WarehouseManagement.tsx               → 合并到BaseSettings（仓库管理）
├── src/pages/TeamManagement.tsx                    → 合并到PersonnelManagement（班组管理）
...
扩展设置：
├── src/pages/CostAccounting.tsx                    → 保留（升级为SQLite）
├── src/pages/AuditLog.tsx                          → 保留（升级为操作审计）
├── src/pages/FarmActivityManagement.tsx            → 合并到ProcessManagement（农事工序）
```
