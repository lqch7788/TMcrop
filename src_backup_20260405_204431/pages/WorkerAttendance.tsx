...
import { Users, Search, Download, Eye, ChevronLeft, ChevronRight, X } from 'lucide-react';

const attendanceData = [
  { id: 1, workerId: 'W001', name: '张伟民', dept: '生产部', date: '2024-03-14', checkIn: '07:50', checkOut: '18:10', hours: 10, status: '正常', statusClass: 'normal' },
  { id: 2, workerId: 'W002', name: '李明轩', dept: '生产部', date: '2024-03-14', checkIn: '08:00', checkOut: '18:00', hours: 10, status: '正常', statusClass: 'normal' },
  { id: 3, workerId: 'W003', name: '王建国', dept: '生产部', date: '2024-03-14', checkIn: '07:45', checkOut: '17:50', hours: 10, status: '正常', statusClass: 'normal' },
  { id: 4, workerId: 'W004', name: '赵俊杰', dept: '技术部', date: '2024-03-14', checkIn: '08:10', checkOut: '18:00', hours: 9.5, status: '迟到', statusClass: 'warning' },
  { id: 5, workerId: 'W005', name: '钱文涛', dept: '生产部', date: '2024-03-14', checkIn: '-', checkOut: '-', hours: 0, status: '请假', statusClass: 'draft' },
];
...
export default function WorkerAttendance() {
  ...
  const handleDoExport = async () => {
    ...
    const headers = ['工号', '姓名', '部门', '日期', '签到时间', '签退时间', '工时', '状态'];
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-100">
              {attendanceData.slice((currentPage - 1) * pageSize, currentPage * pageSize).map((att) => (
                <tr key={att.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${
                      att.statusClass === 'normal' ? 'bg-green-100 text-green-700' :
                      att.statusClass === 'warning' ? 'bg-amber-100 text-amber-700' :
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100">
            ...
            <div className="flex items-center gap-2">
              <span className="text-sm text-gray-500">共 {attendanceData.length} 条</span>
              ...
              <span className="text-sm">{currentPage} / {Math.ceil(attendanceData.length / pageSize) || 1}</span>
              <button onClick={() => setCurrentPage(Math.min(Math.ceil(attendanceData.length / pageSize), currentPage + 1))} disabled={currentPage >= Math.ceil(attendanceData.length / pageSize)} className="p-1.5 rounded hover:bg-gray-100 disabled:opacity-50">
