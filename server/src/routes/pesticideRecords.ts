...
router.get('/', (req: Request, res: Response) => {
  try {
    ...
    const items = queryToObjects(db,
      `SELECT * FROM pesticide_records ${whereClause} ORDER BY spray_time DESC LIMIT ? OFFSET ?`,
...
router.put('/:id', (req: Request, res: Response) => {
  try {
    ...
    db.run(`UPDATE pesticide_records SET
      ...
      [body.spray_time ?? existing[0].spray_time, body.operator_name ?? existing[0].operator_name,
       body.crop_name ?? existing[0].crop_name, body.greenhouse_name ?? existing[0].greenhouse_name,
       body.control_type ?? existing[0].control_type,
       body.pesticide_id ?? existing[0].pesticide_id, body.pesticide_name ?? existing[0].pesticide_name,
       body.pesticide_type ?? existing[0].pesticide_type, body.spec_id ?? existing[0].spec_id,
       body.spec_content ?? existing[0].spec_content,
       body.dosage ?? existing[0].dosage, body.dosage_unit ?? existing[0].dosage_unit,
       body.dilution_ratio ?? existing[0].dilution_ratio, body.target_pest ?? existing[0].target_pest,
       body.application_method ?? existing[0].application_method,
       body.bio_agent_id ?? existing[0].bio_agent_id, body.bio_agent_name ?? existing[0].bio_agent_name,
       body.bio_agent_type ?? existing[0].bio_agent_type,
       body.equipment_name ?? existing[0].equipment_name, body.equipment_count ?? existing[0].equipment_count,
       body.use_leaf_fertilizer ?? existing[0].use_leaf_fertilizer,
       body.leaf_fertilizer_name ?? existing[0].leaf_fertilizer_name,
       body.leaf_fertilizer_dosage ?? existing[0].leaf_fertilizer_dosage,
       body.leaf_fertilizer_unit ?? existing[0].leaf_fertilizer_unit,
