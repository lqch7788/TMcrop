...
const ALLOWED_FIELDS = [
  'staff_id', 'staff_name', 'department', 'month',
  'task_completion_rate', 'attendance_rate', 'work_quality',
  'safety_compliance', 'teamwork_attitude', 'total_score',
  'rank', 'status', 'remarks',
];

/** GET /api/performance — 列表（filter: month, department, keyword, page, limit） */
router.get('/', (req: Request, res: Response) => {
  try {
    const db = getDatabase();
    const { month, department, keyword, page = '1', limit = '50' } = req.query;
    const pageNum = Math.max(1, parseInt(page as string, 10) || 1);
    const limitNum = Math.min(500, Math.max(1, parseInt(limit as string, 10) || 50));
    const offset = (pageNum - 1) * limitNum;

    let sql = "SELECT * FROM performance_records WHERE (deleted_at IS NULL OR deleted_at = '')";
    const params: (string | number)[] = [];

    if (month) { sql += ' AND month = ?'; params.push(month as string); }
    if (department) { sql += ' AND department = ?'; params.push(department as string); }
    if (keyword) {
      sql += ' AND (staff_name LIKE ? OR staff_id LIKE ?)';
      const k = `%${keyword}%`;
      params.push(k, k);
    }
    ...
    sql += ' ORDER BY month DESC, create_time DESC LIMIT ? OFFSET ?';
    ...
    res.status(500).json({ success: false, error: '查询考核记录失败' });
  }
});
...
router.get('/:id', (req: Request, res: Response) => {
  try {
    const db = getDatabase();
    const r = db.exec('SELECT * FROM performance_records WHERE id = ? AND (deleted_at IS NULL OR deleted_at = "")', [req.params.id]);
    if (!r[0] || r[0].values.length === 0) {
      return res.status(404).json({ success: false, error: '考核记录不存在' });
    }
    const row = Object.fromEntries(r[0].columns.map((c, i) => [c, r[0].values[0][i]]));
    res.json({ success: true, data: row });
  } catch (e) {
    console.error('[performance GET /:id] error:', e);
    res.status(500).json({ success: false, error: '查询考核记录失败' });
  }
});
 ...
 * 必填：staff_id, staff_name, month
 */
router.post('/', (req: Request, res: Response) => {
  try {
    const body = req.body || {};
    const required = ['staff_id', 'staff_name', 'month'];
    ...
    console.error('[performance POST /] error:', e);
    res.status(500).json({ success: false, error: '新建考核记录失败' });
  }
});
...
router.put('/:id', (req: Request, res: Response) => {
  try {
    const body = req.body || {};
    const db = getDatabase();
    const exist = db.exec('SELECT id FROM performance_records WHERE id = ? AND (deleted_at IS NULL OR deleted_at = "")', [req.params.id]);
    ...
    console.error('[performance PUT /:id] error:', e);
    res.status(500).json({ success: false, error: '更新考核记录失败' });
  }
});

/** DELETE /api/performance/:id — 软删 */
router.delete('/:id', (req: Request, res: Response) => {
  try {
    ...
    console.error('[performance DELETE /:id] error:', e);
    res.status(500).json({ success: false, error: '删除考核记录失败' });
  }
});
