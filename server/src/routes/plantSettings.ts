...
router.get('/', (req, res) => {
  try {
    ...
    if (crop_variety_oid) { conditions.push('crop_variety_oid = ?'); params.push(crop_variety_oid); }
...
router.post('/', (req, res) => {
  try {
    ...
    if (!setting_key) return res.status(400).json({ success: false, error: '设置键不能为空' });
