...
router.put('/:id', (req: Request, res: Response) => {
  try {
    ...
    db.run(`UPDATE pesticide_specs SET
      ...
      [pickBody(body, 'pesticideName', 'pesticide_name') ?? existing[0].pesticideName,
       pesticideTypeValue !== undefined ? pesticideTypeValue : existing[0].pesticideTypes ?? existing[0].pesticide_type,
       pickBody(body, 'ingredient', 'ingredient') ?? existing[0].ingredient,
       pickBody(body, 'mechanism', 'mechanism') ?? existing[0].mechanism,
       pickBody(body, 'functionDesc', 'function_desc') ?? existing[0].functionDesc ?? existing[0].function_desc,
       pickBody(body, 'tabooDesc', 'taboo_desc') ?? existing[0].tabooDesc ?? existing[0].taboo_desc,
       pickBody(body, 'targetPests', 'target_pests') ?? existing[0].targetPests ?? existing[0].target_pests,
       pickBody(body, 'specContent', 'spec_content') ?? existing[0].specContent ?? existing[0].spec_content,
       pickBody(body, 'formulation', 'formulation') ?? existing[0].formulation,
       pickBody(body, 'manufacturer', 'manufacturer') ?? existing[0].manufacturer,
       pickBody(body, 'brandName', 'brand_name') ?? existing[0].brandName ?? existing[0].brand_name,
       pickBody(body, 'suggestedDosage', 'suggested_dosage') ?? existing[0].suggestedDosage ?? existing[0].suggested_dosage,
       pickBody(body, 'suggestedRatio', 'suggested_ratio') ?? existing[0].suggestedRatio ?? existing[0].suggested_ratio,
       pickBody(body, 'dosageUnit', 'dosage_unit') ?? existing[0].dosageUnit ?? existing[0].dosage_unit,
       pickBody(body, 'remark', 'remark') ?? existing[0].remark,
       pickBody(body, 'stockQuantity', 'stock_quantity') != null ? Number(pickBody(body, 'stockQuantity', 'stock_quantity')) : (existing[0].stockQuantity ?? existing[0].stock_quantity ?? 0),
       pickBody(body, 'stockUnit', 'stock_unit') ?? existing[0].stockUnit ?? existing[0].stock_unit ?? 'kg',
       pickBody(body, 'unitPrice', 'unit_price') != null ? Number(pickBody(body, 'unitPrice', 'unit_price')) : (existing[0].unitPrice ?? existing[0].unit_price ?? 0),
       pickBody(body, 'batchNumber', 'batch_number') ?? existing[0].batchNumber ?? existing[0].batch_number,
       pickBody(body, 'productionDate', 'production_date') ?? existing[0].productionDate ?? existing[0].production_date,
       pickBody(body, 'expirationDate', 'expiration_date') ?? existing[0].expirationDate ?? existing[0].expiration_date,
       pickBody(body, 'packageSpec', 'package_spec') ?? existing[0].packageSpec ?? existing[0].package_spec,
       pickBody(body, 'status', 'status') ?? existing[0].status,
       now, id]
    );
