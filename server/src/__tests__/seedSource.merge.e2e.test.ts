...
describe('种源自动合并 E2E', () => {
  ...
  describe('写时合并', () => {
    it('同合并键多次回流 → 合并为同一种源', async () => {
      ...
      // DEBUG: 用 parent 的实际 crop_code 查询（sql.js API）
      console.log('[DEBUG] parent cropCode:', cropCode);
      const debugResult = await seedSourceRepository.findMergeableSeedSource({
        cropCode: cropCode,
        seedForm: '种子',
        unit: '粒',
        generation: 'F1',
      });
      console.log('[DEBUG] findMergeable result:', debugResult);

      // 第一次回流 → 创建新种源
      const result1 = await executeCirculation(baseInput);
      console.log('[DEBUG] result1.mergeAction:', result1.mergeAction, 'stockId:', result1.stockId);
  ...
  describe('matchable 查询', () => {
    it('返回同合并键的候选种源', async () => {
      ...
      // 创建一条 active 种源（同合并键）
      db.run(`INSERT INTO seed_sources (id, source_code, source_origin, crop_code, crop_name, seed_form, unit, generation, quantity, remaining_quantity, status, create_time, update_time)
              VALUES (?, ?, 'planting_self_kept', ?, '匹配作物', '果实', '粒', 'F1', 50, 50, 'active', ?, ?)`, [matchId, matchCode, matchCrop, now, now]);
