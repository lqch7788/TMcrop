...
export function createIndexes() {
  ...
  // ========== V12.0: 病虫害防治管理索引 ==========
  // 2026-07-12：药剂库扁平化 — 索引从 pesticide_library + pesticide_specs 改为单一 pesticide_specs
  db.run(`CREATE INDEX IF NOT EXISTS idx_pesticide_code ON pesticide_specs(pesticide_code)`);
  db.run(`CREATE INDEX IF NOT EXISTS idx_pesticide_name ON pesticide_specs(pesticide_name)`);
  // 清理旧索引（pesticide_library 的旧编码索引 + pesticide_specs 的旧 FK 索引）
  try { db.run(`DROP INDEX IF EXISTS idx_pesticide_specs_pesticide_id`); } catch {}
