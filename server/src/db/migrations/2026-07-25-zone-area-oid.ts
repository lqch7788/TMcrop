 ...
 *   3. UPDATE plantings / seedlings.area_oid = zones.oid（按 zone_name 匹配）
...
export function migrate20260725ZoneAreaOid(db: Database): void {
  console.log('[migrate 2026-07-25] 开始 plantings/seedlings.area_oid 迁移');
  ...
  // 反查 zones.oid，按 zone_name 匹配；失败保留为 NULL
  db.run(`
    UPDATE plantings
  ...
  console.log('[migrate 2026-07-25] 完成');
}
