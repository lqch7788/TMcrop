...
/**
 * 2026-08-17：导入标记状态字典（plant_mark_status 分类）
 * - 4 个父节点（长势/事件/状态/品质） + 20 个子节点
 * - 对齐 iAGS 标记截图 3
 * - 用户可在系统设置 → 数据字典 自由增删改
 */
export async function seedMarkStatus() {
  ...
  for (const c of children) {
    ...
    if (!pid) { console.warn('[seedMarkStatus] 找不到父节点 pid=' + c.pid); continue; }
    if (existingChildCodes.has(c.code)) continue; // 已存在则跳过（幂等）
    const dictValue = c.aid ? `${c.code}|${c.aid}` : c.code;
    db.run(
      `INSERT INTO dictionaries (id, category_code, dict_code, dict_label, dict_value, color, parent_id, sort_order, status, created_at, updated_at)
       VALUES (?, 'plant_mark_status', ?, ?, ?, ?, ?, ?, 'active', ?, ?)`,
      [`pm-${c.code}`, c.code, c.label, dictValue, c.color, pid, c.sort, now, now]
    );
    insertedChildren++;
  }
...
function seedFertilizerAndMarks() {
  ...
  // === plant_marks 标记种子数据（保留旧 plant_marks 表，向后兼容老履历数据） ===
  ...
  if ((existingIotDevices[0]?.values[0]?.[0] ?? 0) === 0) {
    const devices = [
      { device_id: 'IOT-FERT-001', device_name: '施肥区1号传感器', api_key: 'iot-test-key-2026', is_active: 1, create_time: new Date().toISOString() },
      { device_id: 'IOT-FERT-002', device_name: '施肥区2号传感器', api_key: 'iot-test-key-2026', is_active: 1, create_time: new Date().toISOString() },
      { device_id: 'IOT-ENV-001', device_name: '环境监测主传感器', api_key: 'iot-test-key-2026', is_active: 1, create_time: new Date().toISOString() },
    ];
...
function seedPesticideRecords() {
  ...
  const now = new Date().toISOString();
  // 2026-07-10：pesticide_type 改为 dictCode（中文 → 英文 code），移除 control_type 字段
  const records = [
    { id: 'PEST_001', record_code: 'PC2026050201', spray_time: '2026-05-02 08:00:00', operator_id: 'WORKER_001', operator_name: '张三', crop_name: '红生菜', greenhouse_name: '1号温室', pesticide_id: 'PEST_CHEM_001', pesticide_name: '百菌清', pesticide_type: '["fungicide_fungi"]', spec_id: '', spec_content: '75%可湿性粉剂', dosage: 100, dosage_unit: 'g/亩', dilution_ratio: '1:800', target_pest: '霜霉病', application_method: '叶面喷雾', bio_agent_id: '', bio_agent_name: '', bio_agent_type: '', equipment_name: '背负式喷雾器', equipment_count: '2', use_leaf_fertilizer: 'no', leaf_fertilizer_name: '', leaf_fertilizer_dosage: 0, leaf_fertilizer_unit: '', description: '预防性喷施', photos: '', status: 'completed' },
    { id: 'PEST_002', record_code: 'PC2026050501', spray_time: '2026-05-05 09:30:00', operator_id: 'WORKER_002', operator_name: '李四', crop_name: '菠菜', greenhouse_name: '2号温室', pesticide_id: 'PEST_CHEM_002', pesticide_name: '吡虫啉', pesticide_type: '["insecticide_sucking"]', spec_id: '', spec_content: '10%可湿性粉剂', dosage: 50, dosage_unit: 'g/亩', dilution_ratio: '1:2000', target_pest: '蚜虫', application_method: '叶面喷雾', bio_agent_id: '', bio_agent_name: '', bio_agent_type: '', equipment_name: '背负式喷雾器', equipment_count: '2', use_leaf_fertilizer: 'no', leaf_fertilizer_name: '', leaf_fertilizer_dosage: 0, leaf_fertilizer_unit: '', description: '治疗性喷施', photos: '', status: 'completed' },
    { id: 'PEST_003', record_code: 'PC2026050801', spray_time: '2026-05-08 07:00:00', operator_id: 'WORKER_003', operator_name: '王五', crop_name: '番茄', greenhouse_name: '3号温室', pesticide_id: 'PEST_BIO_001', pesticide_name: '枯草芽孢杆菌', pesticide_type: '["fungicide_fungi"]', spec_id: '', spec_content: '100亿活芽孢/g', dosage: 200, dosage_unit: 'g/亩', dilution_ratio: '1:500', target_pest: '土传病害', application_method: '灌根', bio_agent_id: 'BIO_001', bio_agent_name: '枯草芽孢杆菌', bio_agent_type: '有益菌', equipment_name: '灌溉系统', equipment_count: '1', use_leaf_fertilizer: 'no', leaf_fertilizer_name: '', leaf_fertilizer_dosage: 0, leaf_fertilizer_unit: '', description: '生物防治', photos: '', status: 'completed' },
    { id: 'PEST_004', record_code: 'PC2026051001', spray_time: '2026-05-10 10:00:00', operator_id: 'WORKER_001', operator_name: '张三', crop_name: '黄瓜', greenhouse_name: '4号温室', pesticide_id: 'PEST_CHEM_003', pesticide_name: '霜脲氰', pesticide_type: '["fungicide_fungi"]', spec_id: '', spec_content: '50%水分散粒剂', dosage: 80, dosage_unit: 'g/亩', dilution_ratio: '1:1500', target_pest: '霜霉病', application_method: '叶面喷雾', bio_agent_id: '', bio_agent_name: '', bio_agent_type: '', equipment_name: '电动喷雾器', equipment_count: '3', use_leaf_fertilizer: 'yes', leaf_fertilizer_name: '磷酸二氢钾', leaf_fertilizer_dosage: 100, leaf_fertilizer_unit: 'g/亩', description: '综合防治', photos: '', status: 'completed' },
  ];
  // 2026-07-10：移除 control_type 列（已 DROP），INSERT 字段列表同步删
  // 2026-07-17：移除 status 列（DB 已 DROP，业务上防治记录无中间态）
  for (const item of records) {
    db.run(`INSERT INTO pesticide_records (id, record_code, spray_time, operator_id, operator_name, crop_name, greenhouse_name, pesticide_id, pesticide_name, pesticide_type, spec_id, spec_content, dosage, dosage_unit, dilution_ratio, target_pest, application_method, bio_agent_id, bio_agent_name, bio_agent_type, equipment_name, equipment_count, use_leaf_fertilizer, leaf_fertilizer_name, leaf_fertilizer_dosage, leaf_fertilizer_unit, description, photos, create_time, update_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [item.id, item.record_code, item.spray_time, item.operator_id, item.operator_name, item.crop_name, item.greenhouse_name, item.pesticide_id, item.pesticide_name, item.pesticide_type, item.spec_id, item.spec_content, item.dosage, item.dosage_unit, item.dilution_ratio, item.target_pest, item.application_method, item.bio_agent_id, item.bio_agent_name, item.bio_agent_type, item.equipment_name, item.equipment_count, item.use_leaf_fertilizer, item.leaf_fertilizer_name, item.leaf_fertilizer_dosage, item.leaf_fertilizer_unit, item.description, item.photos, now, now]);
  }
